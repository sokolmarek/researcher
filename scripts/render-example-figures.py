...
Toolchain (discovered at run time, all optional except the one you need):
    ...
    any TeX engine          for the LaTeX figures (tectonic, or latexmk/pdflatex
                            from TeX Live, MiKTeX, or MacTeX; set LATEX_ENGINE
                            to choose)
...
def render_latex(body, png, engine):
    if engine is None:
    ...
    with tempfile.TemporaryDirectory(prefix="render-") as tmp:
        ...
        ok, output, pdf = compile_document(tex, engine=engine)
        if not ok:
            print(f"  {engine.name} failed:")
            print("\n".join(output.splitlines()[-12:]))
            return False
        return pdf_to_png(pdf, png)
...
def main():
    ...
    try:
        ...
        print(install_hint())
        return 1
    if engine is None and any(TARGETS[n][1] == "latex" for n in names if n in TARGETS):
        print(install_hint())
        return 1
    if engine is not None:
        print(f"LaTeX engine: {engine.name} ({engine.executable})\n")
    ...
    for name in names:
        ...
        if index >= len(found):
            ...
            continue
        png = ASSETS / png_name
        ok = (render_latex(found[index], png, engine) if language == "latex"
