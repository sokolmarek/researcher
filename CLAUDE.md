...
- **Workflows** (`.github/workflows/`): `validate.yml` (official `claude plugin validate`, JSON parse, em-dash and placeholder guards, freshness eval, `cffconvert` validation of `CITATION.cff`), `core.yml` (ruff, mypy, and the offline core suite on a Windows, macOS, and Linux matrix, plus the benchmark runners), `docs-deploy.yml` (GitHub Pages), `release.yml` (runs the full validation suite, checks that the tag, `plugin.json`, `marketplace.json`, CHANGELOG, and `CITATION.cff` agree, generates CycloneDX SBOMs for the kernel and the Word toolchain, signs the archive and SBOMs with Sigstore keyless signing via OIDC, cuts the GitHub Release on tags, and publishes `researcher-core` to PyPI via OIDC trusted publishing with the version read from `pyproject.toml`, not the tag).
