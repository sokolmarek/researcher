...
4. **No em dashes in generated text.** Restructure the sentence: use commas, parentheses, colons, or two
   sentences instead.
5. **Compile-check all LaTeX before delivery** with whatever TeX engine is installed: run
   `scripts/latex-compile.py` (or `scripts/latex-compile.sh` on POSIX), which autodetects tectonic,
   latexmk from TeX Live / MiKTeX / MacTeX, or a raw pdflatex. Override the choice with `--engine` or the
   `LATEX_ENGINE` environment variable. Tectonic is the recommended default (one binary, fetches packages
   on demand, reproducible builds) and is what CI uses, but it is a recommendation, not a requirement; a
   full TeX distribution works out of the box. Never deliver a `.tex` file that has not compiled.
