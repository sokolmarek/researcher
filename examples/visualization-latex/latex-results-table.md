...
Compiles with `tectonic` (requires `booktabs`, `multirow`, `threeparttable`). Verified: compiled on
2026-07-14. Every star below is computed from the per-seed input with a paired $t$-test against the
supervised from-scratch baseline at the same labeled fraction (df = 4).
