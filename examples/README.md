...
- **Real citations only.** Every DOI, author, year, venue, and quoted or reported figure comes from an actual retrieval performed while authoring the examples (OpenAlex, Crossref, arXiv, and Scite). No bibliographic detail is entered from memory. Every DOI resolves.
- **One carve-out, and it is labeled at every point of use.** The PRISMA example (`research-verification/literature-search-prisma.md`) carries three sets of numbers that were **not** retrieved: the per-source raw hit counts, the PRISMA stage counts computed from them, and the Scite supporting/contrasting/mentioning tallies (the Scite MCP was rate-limited at authoring). That example calls them *illustrative* and says so beside every table, fence, and JSON block that contains one, and it defines the term against *verified* in a provenance key at the top. Illustrative values show the shape of a field; they are excluded from the verified citation record, and a real run overwrites them. The rule above still holds for everything else in that example, including all of its DOIs and arXiv IDs, which are real and resolve.
