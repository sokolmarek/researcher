...
2. LaTeX freshness: every fenced ```latex/```tex block is classified as
   ...
   (wrapped in a minimal harness document first), then compiled with tectonic.
   The generated fixture at evals/fixtures/manuscript-min/main.tex is compiled
   as a real multi-file manuscript.

Usage:
    python evals/example-freshness.py                # everything
    python evals/example-freshness.py --skip-doi     # offline: LaTeX only
    python evals/example-freshness.py --skip-latex   # no tectonic: DOIs only

Requires only the stdlib. Tectonic is looked up on PATH or via the TECTONIC
environment variable. Exits nonzero if any check fails.
"""
...
def compile_tex(tectonic, workdir, tex_name):
    result = subprocess.run(
        [tectonic, tex_name, "--keep-logs"],
        cwd=workdir,
        capture_output=True,
        text=True,
        encoding="utf-8",
...
def check_latex(blocks, fixture, tectonic):
    failures = []
    with tempfile.TemporaryDirectory(prefix="freshness-") as tmp:
        ...
        for md, index, kind, body in blocks:
            ...
            ok, output = compile_tex(tectonic, block_dir, "block.tex")
    ...
    if fixture is not None:
        if fixture.exists():
            ok, output = compile_tex(tectonic, fixture.parent, fixture.name)
            if ok:
...
def main():
    ...
    if args.skip_latex:
        ...
        if tectonic is None:
            print("LaTeX freshness: FAILED, tectonic not found (PATH or TECTONIC env var)")
            failures.append("tectonic not found; install it or pass --skip-latex")
        else:
            print(f"\nLaTeX freshness (tectonic: {tectonic}):")
            failures += check_latex(blocks, Path(args.fixture) if args.fixture else None, tectonic)
